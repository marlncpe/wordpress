...
if ( ! class_exists( 'Themify_Builder' ) ) {
	...
	class Themify_Builder {
		...
		function init() {
			...
			add_action( 'wp_ajax_get_html_builder', array( &$this, 'get_html_builder_ajaxify' ), 10 );
		 ...
		 */
		function get_html_builder_ajaxify(){
			check_ajax_referer( 'tfb_load_nonce', 'nonce' );
			...
			if ( is_array( $builder_data ) && count( $builder_data ) > 0 ) {
				$html = $this->retrieve_template( 'builder-output.php', array( 'builder_output' => $builder_data, 'builder_id' => $post_id ), '', '', false );
				$html = preg_replace('~>\s+<~', '><', $html);
    			$html = preg_replace('/\s\s+/', ' ', $html);
			}

			$response = array(
				'text_str' => trim( strip_tags( $html ))
			);
		...
		function get_custom_styling( $style_id, $mod_name, $settings, $array = false ) {
			...
			if ( $mod_name != 'row' ) {
				$styles_selector = is_array( $this->modules[ $mod_name ]['styling_selector'] ) ? $this->modules[ $mod_name ]['styling_selector'] : array();
			} else {
