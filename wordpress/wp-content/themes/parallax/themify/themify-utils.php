...
/***************************************************************************/
	 ...
	 * @since 1.7.4
	 */
	function themify_enqueue_framework_stylesheets() {
		// Skin stylesheet
...
function themify_font_icons_admin_assets() {
	wp_enqueue_style( 'themify-font-icons-css', THEMIFY_URI . '/fontawesome/css/font-awesome.min.css', array(), wp_get_theme()->display('Version') );
	wp_enqueue_script( 'themify-font-icons-js', trailingslashit( THEMIFY_URI ) . 'js/themify.font-icons-select.js', array( 'jquery' ) );
...
function themify_get_font_icons_dialog() {
	$icons = '
		<div id="themify_lightbox_fa" class="themify-admin-lightbox clearfix" style="display: none;">
			<h3 class="themify_lightbox_title">' . _e( 'Choose icon', 'themify' ) . '</h3>
...
function themify_setup_menu_icons() {
	if( is_admin() ) {
		...
		add_action( 'wp_enqueue_scripts', 'themify_enqueue_menu_icon_assets' );
	}
}
...
/**
 * Enqueue the icon font only when at least one menu item has an icon
 *
 * @since 1.6.8
 */
function themify_enqueue_menu_icon_assets() {
	$queue_icon_font = false;
...
class Themify_Editor_GFonts {
	...
	public function setup_defaults() {
		$data = themify_get_data();
		if( null === $data[$this->setting_name] ) {
	...
	function config_view( $data = array() ) {
		...
		$output .= '</select><input type="hidden" name="'. $this->setting_name .'" id="'. $this->setting_name .'" value=\''. json_encode( $fonts ) .'\' />';
	...
	function mce_external_plugins( $plugin_array ) {
		$plugin_array['themifyCustomFonts'] = THEMIFY_URI . '/js/tinymce.customfonts.js';
