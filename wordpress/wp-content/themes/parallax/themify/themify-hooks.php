...
class Themify_Hooks {
			...
			foreach( $this->get_location_groups() as $group => $label ) {
				if( ! empty( $locations[$group] ) ) {
					...
					foreach( $locations[$group] as $key => $value ) {
						$output .= '<option value="' . $key . '" ' . selected( $data["{$this->pre}-{$id}-location"], $key, false ) . '>' . $value . '</option>';
					}
					$output .= '</optgroup>';
				}
			}
			$output .= '</select>';
			$output .= '&nbsp; <a class="button button-secondary themify-visibility-toggle" href="#" data-target="#'. $this->pre .'-'. $id .'-visibility"> ' . __( '+ Conditions', 'themify' ) . ' </a> <input type="hidden" id="'. $this->pre .'-'. $id .'-visibility" name="'. $this->pre .'-'. $id .'-visibility" value=\''. $data["{$this->pre}-{$id}-visibility"] .'\' /></div>';
			$output .= '<div class="row"><textarea name="'. $this->pre .'-'.$id.'-code" class="" rows="6" cols="73">'. esc_html( $data["{$this->pre}-{$id}-code"] ) .'</textarea>';
	...
	public function get_visibility_options( $selected = array() ) {
		...
		$output .= '<div id="visibility-tab-general" class="themify-visibility-options clearfix">';
			$output .= '<label><input type="checkbox" name="general[home]" '. checked( $selected['general']['home'], 'on', false ) .' />' . __( 'Home page', 'themify' ) . '</label>';
			$output .= '<label><input type="checkbox" name="general[page]" '. checked( $selected['general']['page'], 'on', false ) .' />' . __( 'Page views', 'themify' ) . '</label>';
			$output .= '<label><input type="checkbox" name="general[single]" '. checked( $selected['general']['single'], 'on', false ) .' />' . __( 'Single post views', 'themify' ) . '</label>';
			$output .= '<label><input type="checkbox" name="general[search]" '. checked( $selected['general']['search'], 'on', false ) .' />' . __( 'Search pages', 'themify' ) . '</label>';
			$output .= '<label><input type="checkbox" name="general[category]" '. checked( $selected['general']['category'], 'on', false ) .' />' . __( 'Category archive', 'themify' ) . '</label>';
			$output .= '<label><input type="checkbox" name="general[tag]" '. checked( $selected['general']['tag'], 'on', false ) .' />' . __( 'Tag archive', 'themify' ) . '</label>';
			$output .= '<label><input type="checkbox" name="general[author]" '. checked( $selected['general']['author'], 'on', false ) .' />' . __( 'Author pages', 'themify' ) . '</label>';
		...
		$output .= '<div id="visibility-tab-post-types" class="themify-visibility-options clearfix">';
			...
			foreach( $post_types as $key => $post_type ) {
				...
				if( ! empty( $posts ) ) : foreach( $posts as $post ) :
					...
					$output .= '<label><input type="checkbox" name="post_type[' . $key . ']['. $post->post_name .']"'. checked( $selected['post_type'][$key][$post->post_name], 'on', false ) .' />' . $post->post_title . '</label>';
				endforeach; endif;
				$output .= '</div>';
			}
			$output .= '</div>';
		...
		$output .= '<div id="visibility-tab-taxonomies" class="themify-visibility-options clearfix">';
			...
			foreach( $taxonomies as $key => $tax ) {
				...
				if( ! empty( $terms ) ) : foreach( $terms as $term ) :
					$output .= '<label><input type="checkbox" name="tax['. $key .']['. $term->slug .']" '. checked( $selected['tax'][$key][$term->slug], 'on', false ) .' />' . $term->name . '</label>';
				endforeach; endif;
				$output .= '</div>';
			}
			$output .= '</div>';
		...
		foreach( $GLOBALS['wp_roles']->roles as $key => $role ) {
			$output .= '<label><input type="checkbox" name="roles['. $key .']" '. checked( $selected['roles'][$key], 'on', false ) .' />' . $role['name'] . '</label>';
		}
