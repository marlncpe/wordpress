...
<?php else : ?>
	...
	<form action="<?php echo esc_url( add_query_arg( 'address', $load_address, get_permalink( woocommerce_get_page_id('edit_address') ) ) ); ?>" method="post">
		...
		<h3 class="woo_title"><?php if ($load_address=='billing') _e( 'Billing Address', 'themeton' ); else _e( 'Shipping Address', 'themeton' ); ?></h3>
		...
		<p>
			<input type="submit" class="button" name="save_address" value="<?php _e( 'Save Address', 'themeton' ); ?>" />
